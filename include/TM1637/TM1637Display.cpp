...
#include "../bcmwrapper/bcmwrapper.h"
...
TM1637Display::TM1637Display(uint8_t pinClk, uint8_t pinDIO, unsigned int bitDelay)
{
    ...
    gpio_pinmode(m_pinClk, INPUT);
    gpio_pinmode(m_pinDIO,INPUT);
	gpio_write(m_pinClk, LOW);
	gpio_write(m_pinDIO, LOW);
}
